...
class UpdateEntryType
{
    ...
    ): array
    {
        ...
        if ($handle !== null) {
            // Validate handle uniqueness (excluding current entry type)
            $existingEntryType = $entriesService->getEntryTypeByHandle($handle);
            if ($existingEntryType && $existingEntryType->id !== $entryTypeId) {
        ...
        if ($hasTitleField !== null) {
            ...
            // Manage field layout based on hasTitleField setting
            $fieldLayout = $entryType->getFieldLayout();
            if ($hasTitleField && !$fieldLayout->isFieldIncluded('title')) {
                // Add title field to layout
                $fieldLayout->prependElements([new EntryTitleField()]);
            } elseif (!$hasTitleField && $fieldLayout->isFieldIncluded('title')) {
                // Remove title field from layout - work with tabs
                $tabs = $fieldLayout->getTabs();
                foreach ($tabs as $tab) {
                    $elements = $tab->getElements();
                    $filteredElements = array_filter($elements, function($element) {
                        return !($element instanceof EntryTitleField);
                    });
                    if (count($filteredElements) !== count($elements)) {
                        $tab->setElements($filteredElements);
                    }
                }
            }
        }
